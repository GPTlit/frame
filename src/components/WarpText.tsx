...
export type WarpTextProps = Partial<TextProps> & {
  className?: string;
  style?: CSSProperties;
};
...
export default function WarpText({
  ...
  const contextRef = useRef<{ program: AnyProgram; rasterize: () => void } | null>(null);
  ...
  useEffect(() => {
    ...
    let gl: WebGL2RenderingContext & { canvas: HTMLCanvasElement };
    try {
    ...
    const texture = new Texture(gl as never, {
    ...
    const program = new Program(gl as never, {
      vertex,
      fragment,
      transparent: true,
      depthTest: false,
    ...
    const uniforms = program.uniforms as Record<string, { value: never }>;
    ...
    const rasterize = async () => {
      ...
      texture.image = textCanvas as never;
    ...
    syncUniforms(program as never, propsRef.current);
    contextRef.current = { program: program as never, rasterize: () => void rasterize() };
    resize();
    raf = requestAnimationFrame(loop);
